...
| **Settings → Technical → Outgoing Mail Servers** | the committee mailbox. Most providers refuse any other sender, so the module (install/upgrade) sets *FROM Filtering* to the company email address and creates an *Alias Domain* whose default From, catchall and bounce are that mailbox: mails from OdooBot or the public website user then go out as the committee address, with the author's name, and replies land in the mailbox. The company email (**Settings → Companies**) must be the mailbox address |
