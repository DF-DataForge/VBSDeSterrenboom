...
| **Settings → Companies → Email** | the committee mailbox address (`ocdesterrenboom@gmail.com`). Filled in by the module from the outgoing mail server's account when empty; setting or changing it re-applies the outgoing mail identity below |
| **Settings → Technical → Outgoing Mail Servers** | the committee mailbox. Most providers refuse any other sender, so the module sets *FROM Filtering* to the company email address and creates an *Alias Domain* whose default From, catchall and bounce are that mailbox: mails from OdooBot or the public website user then go out as the committee address, with the author's name, and replies land in the mailbox. A *personal* server for that mailbox (with an *Owner*, as created from a user's preferences for Gmail/Outlook) is made company-wide, otherwise Odoo would only use it for that user's own mails |
